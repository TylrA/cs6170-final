...
class PersistenceWeightedGaussianKernel(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, bandwidth=1.0, weight=lambda x: 1, kernel_approx=None, use_pss=False):
        ...
        self.kernel_approx_ = kernel_approx
    ...
    def transform(self, X):
        ...
        if np.array_equal(list(self.diagrams_), Xp):
            print("Constructing Persistence Scale Space Kernel")
            counter = 0
            num = (len(self.diagrams_) * (len(self.diagrams_) - 1)) // 2
            sys.stdout.write('0 / %s' % str(num))
            sys.stdout.flush()
            if self.kernel_approx_ is not None:
                ...
                for i in range(len(self.diagrams_)):
                    for j in range(i+1, len(self.diagrams_)):
                        ...
                        Xfit[j,i] = Xfit[i,j]
                        counter += 1
                        sys.stdout.write('\r%s / %s' % (str(counter), str(num)))
                        sys.stdout.flush()
        else:
            # print("Processing prediction queries")
            counter = 0
            num = len(Xp) * len(self.diagrams_)
            sys.stdout.write('0 / %s' % str(num))
            sys.stdout.flush()
            ws = [ np.array([self.weight_(Xp[i][j,:]) for j in range(Xp[i].shape[0])]) for i in range(len(Xp)) ]
            if self.kernel_approx_ is not None:
                approx = np.concatenate([np.sum(np.multiply(ws[i][:,np.newaxis], self.kernel_approx_.transform(Xp[i])), axis=0)[np.newaxis,:] for i in range(len(Xp))])
...
class PersistenceFisherDistance(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, bandwidth=1.0, kernel_approx=None):
        self.bandwidth_, self.kernel_approx_ = bandwidth, kernel_approx
    ...
    def fit(self, X, y=None):
        ...
        if self.kernel_approx_ is not None:
            self.approx_ = [self.kernel_approx_.transform(X[i]) for i in range(len(X))]
    ...
    def transform(self, X):
        ...
        if self.diagrams_ == X:
            for i in range(len(self.diagrams_)):
                for j in range(i+1, len(self.diagrams_)):
                    if self.kernel_approx_ is not None:
                        ...
                        vectori = np.sum(np.exp(-np.square(pairwise_distances(Z,U))/(2 * np.square(self.bandwidth_)))/(self.bandwidth_ * np.sqrt(2*np.pi)), axis=1)
                        vectorj = np.sum(np.exp(-np.square(pairwise_distances(Z,V))/(2 * np.square(self.bandwidth_)))/(self.bandwidth_ * np.sqrt(2*np.pi)), axis=1)
                        vectori, vectorj = vectori/np.sum(vectori), vectorj/np.sum(vectorj)
                        Xfit[i,j] = np.arccos(np.dot(np.sqrt(vectori), np.sqrt(vectorj)))
                        Xfit[j,i] = Xfit[i,j]
        else:
            ...
            if self.kernel_approx_ is not None:
                approx = [self.kernel_approx_.transform(X[i]) for i in range(len(X))]
                approx_diagonal = [self.kernel_approx_.transform(diagonal_projections[i]) for i in range(len(X))]
